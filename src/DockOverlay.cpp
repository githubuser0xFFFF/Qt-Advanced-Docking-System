...
public:
    CDropIndicatorWidget(DockOverlayCrossPrivate* crossPrivate,
                         DockWidgetArea dockWidgetArea,
                         CDockOverlay::eMode mode,
                         QLabel* parent = nullptr)
        : QLabel(parent),
          m_CrossPrivate(crossPrivate),
          m_DockWidgetArea(dockWidgetArea),
          m_Mode(mode)
    {
        Q_ASSERT(m_CrossPrivate != nullptr);
    ...
    void resetPixmap()
    {
        setPixmap(createHighDpiDropIndicatorPixmap());
    }
...
private:
     ...
     * operating system
     */
    qreal dropIndicatiorWidth(QWidget* w) const
    {
    #ifdef Q_OS_LINUX
        Q_UNUSED(w)
        return 40;
    #else
        return static_cast<qreal>(w->fontMetrics().height()) * 3.f;
    #endif
    }

    QPixmap createHighDpiDropIndicatorPixmap(bool hover = false);

private:
    DockOverlayCrossPrivate* m_CrossPrivate = nullptr;
...
public:
    ...
    {
        if (event->type() == QEvent::HoverEnter) qDebug() << "HOVER Enter!!!!!!!!!!!!";
        if (event->type() == QEvent::HoverLeave) qDebug() << "HOVER Leave!!!!!!!!!!!!!";
        if (event->type() == QEvent::HoverMove) qDebug() << "HOVER Move!!!!!!!!!!!!!";
        if (event->type() == QEvent::Enter) qDebug() << "Enter!!!!!!!!!!!!";
        if (event->type() == QEvent::Leave) qDebug() << "Leave!!!!!!!!!!!!!";
        if (event->type() == QEvent::Move) qDebug() << "Move!!!!!!!!!!!!!";
