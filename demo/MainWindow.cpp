...
void MainWindowPrivate::createContent()
{
	...
	{
		auto clicked_DockWidget = createEditorWidget();
		clicked_DockWidget->setFeature(ads::CDockWidget::DockWidgetDeleteOnClose, true);
		DockManager->addDockWidgetTabToArea(clicked_DockWidget, DockArea);
		_this->connect(clicked_DockWidget, SIGNAL(closeRequested()), SLOT(onEditorCloseRequested()));
	});
	...
	for (auto m_DockWidget : DockManager->dockWidgetsMap())
	{
		_this->connect(m_DockWidget, SIGNAL(viewToggled(bool)), SLOT(onViewToggled(bool)));
		_this->connect(m_DockWidget, SIGNAL(visibilityChanged(bool)), SLOT(onViewVisibilityChanged(bool)));
	}
}
